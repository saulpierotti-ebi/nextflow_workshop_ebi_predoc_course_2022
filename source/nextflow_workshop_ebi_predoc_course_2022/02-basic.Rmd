...
And the definition of the `duplicate_lines` and `append_to_file` processes as follows
...
Let's examine how did we achieve this result:
...
- We modified all the processes to expect in input not a single element but tuples of two or three elements in a single channel
  ...
  - When a channel emits lists of elements, the input specifier `tuple` should be used
...
- The input channel is used again to extract the content of the columns `id` and `content`, as lists of two elements (`input_ch.map{ [ it["id"], it["content"] ] }`)
- The channel operator `join` is used to join together the output of the process `duplicate_lines` with the channel resulting from the previous operation (`join(duplicate_lines.out, by: 0)`)
  ...
  - The resulting channel after the joining operation contains lists of three elements: the matching keys itself, the rest of the list for the first channel (one element), and finally the rest of the list for the second channel (one element)
  - The channel is named `append_to_file_in_ch` (`set{ append_to_file_in_ch }`)
